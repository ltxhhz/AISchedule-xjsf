async function scheduleHtmlProvider(iframeContent = '', frameContent = '', dom = document) {
  ...
  if (!content) {
    ...
    if (!configDiv) {
      configDiv = frameDom.getElementById('kbtable').appendChild(
        (() => {
    ...
    if (!configObj.isTeacher) {
      ...
      configObj.delTitle = !!userConfirm
    }
    if (Array.from(frameDom.querySelectorAll('#kbtable [title=教室]')).some(e=>e.innerText.includes('校区'))) {
      
      /** @type {boolean} */
      const userConfirm = await AIScheduleConfirm({
        titleText: '提示',
        contentText: '是否隐藏教室校区信息\n点击后请等待一会',
        cancelText: '保留',
        confirmText: '隐藏'
      })
      configObj.delCampus = !!userConfirm
    }
